-- Materialized View: mv_leaderboard_stats_1_5_4_attachment_filtered
-- Description: Pre-aggregated leaderboard statistics for evaluator version 1.5.4
...
--
-- Usage: This view is queried by the leaderboard API when evaluator version 1.5.4 
--        and "attachment_filtered" mode are selected
--
-- Refresh: Run REFRESH MATERIALIZED VIEW CONCURRENTLY mv_leaderboard_stats_1_5_4_attachment_filtered;
--          after new evaluations are added to keep the data current.

-- Drop the materialized view if it exists
DROP MATERIALIZED VIEW IF EXISTS mv_leaderboard_stats_1_5_4_attachment_filtered CASCADE;

-- Create the materialized view
CREATE MATERIALIZED VIEW mv_leaderboard_stats_1_5_4_attachment_filtered AS
SELECT
  gq.model,
...
CREATE INDEX idx_mv_leaderboard_1_5_4_af_subject_grade 
  ON mv_leaderboard_stats_1_5_4_attachment_filtered (subject, grade_level);

CREATE INDEX idx_mv_leaderboard_1_5_4_af_experiment 
  ON mv_leaderboard_stats_1_5_4_attachment_filtered (experiment_tracker);

CREATE INDEX idx_mv_leaderboard_1_5_4_af_difficulty 
  ON mv_leaderboard_stats_1_5_4_attachment_filtered (difficulty);

CREATE INDEX idx_mv_leaderboard_1_5_4_af_question_type 
  ON mv_leaderboard_stats_1_5_4_attachment_filtered (question_type);

-- Comments
COMMENT ON MATERIALIZED VIEW mv_leaderboard_stats_1_5_4_attachment_filtered IS 
  'Leaderboard statistics for evaluator v1.5.4 filtered to EXCLUDE 48 recipes that require image attachments. Excluded recipe IDs: 1117, 840, 904, 910, 934, 955, 1073, 1096, 1110, 1121, 841, 1400, 1696, 1786, 1979, 1185, 842, 843, 844, 855, 856, 871, 873, 881, 882, 883, 896, 898, 966, 920, 929, 939, 940, 988, 989, 990, 991, 1088, 1130, 1172, 1337, 1345, 1419, 1432, 1514, 1651, 1873, 1997. All other recipes are included (text-only generation).';

COMMENT ON COLUMN mv_leaderboard_stats_1_5_4_attachment_filtered.questions_above_threshold IS 
  'Count of questions with evaluator_score >= 0.85';

COMMENT ON COLUMN mv_leaderboard_stats_1_5_4_attachment_filtered.percentage IS 
  'Success rate: (questions_above_threshold / total_questions) * 100, rounded to 1 decimal place';

COMMENT ON COLUMN mv_leaderboard_stats_1_5_4_attachment_filtered.last_updated IS 
  'Timestamp of most recent evaluation in this group';

-- Grant permissions (adjust as needed for your setup)
-- GRANT SELECT ON mv_leaderboard_stats_1_5_4_attachment_filtered TO your_app_user;
